...
impl<T: ?Sized> NodePtr<T> {
    ...
    pub unsafe fn get_extended_ref<'a>(self) -> &'a Node<T> { unsafe {
        self.0.as_ref()
    }}

    /// # Safety
    /// Pointer must be convertible to a reference
    pub unsafe fn as_ref(&self) -> &Node<T> { unsafe {
        self.0.as_ref()
    }}

    /// # Safety
    /// Pointer must be convertible to a reference
    pub unsafe fn link(&self) -> &Link<T> { unsafe {
        (*self.0.as_ptr()).link.get()
    }}

    /// # Safety
    /// Pointer must be convertible to a reference
    pub unsafe fn link_extended<'a>(self) -> &'a Link<T> { unsafe {
        (*self.0.as_ptr()).link.get()
    }}
}
